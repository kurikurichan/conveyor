...
export interface ModelFilterProps extends ComponentProps<typeof Card> {
  ...
  tableViewOptions: {
    tableView: TableView;
    onTableViewChange: (value: React.SetStateAction<TableView>) => void;
  };
  handleOnSave?: (onSave: () => void) => void;
  handleOnClear?: (onClear: () => void) => void;
}

//field is now field and field options (import type field options)
...
export const ModelFilter = ({
  ...
  // contains array of objects that have a .displayValue property (showing they are filterable)
  const filterableFields = fields.filter((field) => {
    console.log(field);
    return Object.values(ScalarType as Record<string, string>).includes(
      field.fieldOptions,
    );
  });

  // array of objects made to have label and value of the specific fields.
  ...
  // takes a group number and index number and modifies the filters and tableview based on that change.
    ...
    <Card {...cardProps}>
      ...
      <CardFooter className='justify-between'>
        ...
        <Button
          onClick={() =>
            handleOnSave(() =>
              onTableViewChange((oldTableView: TableView) =>
